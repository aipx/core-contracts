...
import {IAlpRewardDistributor} from "../interfaces/IAlpRewardDistributor.sol";
import {ReentrancyGuardUpgradeable} from "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";

contract AgoStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC20 public ago;
    IAlpRewardDistributor public alpRewardDistributor;
    uint256 public lastUpdateRewardTime;
    uint256 public accRewardPerShare;
    mapping(address => UserInfo) public userInfo;

    function initialize(address _ago, address _alpRewardDistributor) external initializer {
        require(_ago != address(0), "AgoStaking:initialize: invalid AGO");
        require(_alpRewardDistributor != address(0), "AgoStaking:initialize: invalid reward distributor");
        __Ownable_init();
        __ReentrancyGuard_init();
        ago = IERC20(_ago);
        alpRewardDistributor = IAlpRewardDistributor(_alpRewardDistributor);
    }

    /*==================================== VIEWS ==================================*/
    function getRewardToken() external view returns (address) {
        return alpRewardDistributor.rewardToken();
    }

    function pendingRewards(address _to) external view returns (uint256) {
        UserInfo memory user = userInfo[_to];
        uint256 _accRewardPerShare = accRewardPerShare;
        uint256 _totalStakedAmount = ago.balanceOf(address(this));
        if (block.timestamp > lastUpdateRewardTime && _totalStakedAmount != 0) {
            uint256 reward = (block.timestamp - lastUpdateRewardTime) * alpRewardDistributor.rewardsPerSecond();
    ...
    function getRewardsPerSecond() external view returns (uint256) {
        return alpRewardDistributor.rewardsPerSecond();
    }

    function withdrawableTokens(address _token) external view returns (bool) {
        return alpRewardDistributor.withdrawableTokens(_token);
    }
    ...
    function stake(address _to, uint256 _amount) external nonReentrant {
        ...
        ago.safeTransferFrom(sender, address(this), _amount);
    ...
    function unstake(address _to, uint256 _amount) external nonReentrant {
        ...
        require(_userInfo.amount >= _amount, "AgoStaking::unstake: unstake amount > staked amount");
        updateRewards();
        _userInfo.amount -= _amount;
        _userInfo.rewardDebt -= int256((_amount * accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function claimRewards(address _to) external nonReentrant {
        ...
        if (_pendingRewards != 0) {
            alpRewardDistributor.transferRewards(_to, _pendingRewards);
    ...
    function claimRewardsToSingleToken(address _to, address _rewardTokenOut, uint256 _minAmountOut)
        ...
        nonReentrant
    {
        require(_rewardTokenOut != address(0), "AgoStaking::claimRewards: Invalid address!");
        updateRewards();
        address sender = msg.sender;
        UserInfo memory user = userInfo[sender];
        int256 _accumulatedReward = int256((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION);
        ...
        userInfo[sender] = user;
        if (_pendingRewards != 0) {
            alpRewardDistributor.transferRewardsToSingleToken(_to, _pendingRewards, _rewardTokenOut, _minAmountOut);
    ...
    function setAlpRewardDistributor(address _alpRewardDistributor) external onlyOwner {
        require(_alpRewardDistributor != address(0), "AgoStaking::setAlpRewardDistributor: Invalid address");
        alpRewardDistributor = IAlpRewardDistributor(_alpRewardDistributor);
        emit AlpRewardDistributorSet(_alpRewardDistributor);
    }
    ...
    event AlpRewardDistributorSet(address indexed _alpRewardDistributor);
}
