...
contract AipxMaster is Ownable, ReentrancyGuard {
    ...
    IPool public immutable aipxPool;
    IAIPXStake public immutable aipxStake;
    IWETH public immutable weth;
    IERC20 public rewardToken;

    /// @notice Info of each user that stakes LP tokens.
    /// pool => userAddress => UserInfo.
    ...
    constructor(address _aipxPool, address _aipxStake, address _weth, address _rewardToken) {
        aipxPool = IPool(_aipxPool);
        aipxStake = IAIPXStake(_aipxStake);
        weth = IWETH(_weth);
    ...
    /// @param _rewardPerSecond The amount of AIPX to be distributed per second.
    ...
    /// @return pending AIPX reward for a given user.
        ...
        nonReentrant
    {
        ...
        IERC20(assetToken).safeIncreaseAllowance(address(aipxPool), assetAmount);
        aipxPool.addLiquidity(tranche, assetToken, assetAmount, minLpAmount, address(this));
    ...
    function addLiquidityETH(uint256 pid, uint256 minLpAmount, address to) external payable nonReentrant {
        ...
        weth.safeIncreaseAllowance(address(aipxPool), _amountIn);
        aipxPool.addLiquidity(tranche, address(weth), _amountIn, minLpAmount, address(this));
        uint256 lpAmount = ILPToken(tranche).balanceOf(address(this)) - balanceLpTokenBefore;
        ...
        nonReentrant
    {
        ...
        tranche.safeIncreaseAllowance(address(aipxPool), lpAmount);
        aipxPool.removeLiquidity(address(tranche), toToken, lpAmount, minOut, to);
    }
    ...
    function removeLiquidityETH(uint256 pid, uint256 lpAmount, uint256 minOut, address to) external nonReentrant {
        ...
        aipxPool.removeLiquidity(address(tranche), address(weth), lpAmount, minOut, address(this));
    ...
    function _transferReward(address _to, uint256 _amount, bool _staking) internal {
        if (_staking) {
            rewardToken.safeIncreaseAllowance(address(aipxStake), _amount);
            aipxStake.stake(_to, _amount);
        } else {
