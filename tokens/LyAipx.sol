...
contract LyAipx is Initializable, OwnableUpgradeable, IERC20 {
    ...
    string public constant name = "Aipx Loyalty Token";
    string public constant symbol = "lyAIPX";
    ...
    function claimRewards(uint256 _batchId, address _receiver) external {
        ...
        require(amount > 0, "LyAipx: nothing to claim");
        _rewards[_batchId][sender] += amount;
        if (enableStaking) {
            rewardToken.safeIncreaseAllowance(address(aipxStake), amount);
    ...
    function setMinter(address _minter) external onlyOwner {
        require(_minter != address(0), "LyAipx: zero address");
        minter = _minter;
