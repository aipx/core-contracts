...
import {IAipxReferralRegistry} from "../interfaces/IAipxReferralRegistry.sol";
import {IAIPXOracle} from "../interfaces/IAIPXOracle.sol";

contract AipxReferralController is Initializable, OwnableUpgradeable {
    ...
    IAipxReferralRegistry public referralRegistry;

    TierInfo[] public tiers;
    /// dev epoch => epochInfo
    mapping(uint256 => EpochInfo) public epochs;
    /// dev epoch => userAddress => userInfo
    ...
    function initialize(address _aipx, address _oracle, address _aipxRegistry, uint256 _epochDuration)
        external
        initializer
    {
        require(_aipx != address(0), "AipxReferralController::initialize: invalid address");
        require(_oracle != address(0), "AipxReferralController::initialize: invalid address");
        require(_aipxRegistry != address(0), "AipxReferralController::initialize: invalid address");
        require(_epochDuration >= MIN_EPOCH_DURATION, "AipxReferralController::initialize: must >= MIN_EPOCH_DURATION");
        __Ownable_init();
        AIPX = IERC20(_aipx);
        oracle = IAIPXOracle(_oracle);
        referralRegistry = IAipxReferralRegistry(_aipxRegistry);
    ...
    function updatePoint(address _trader, uint256 _point) external {
        require(msg.sender == updater, "AipxReferralController::updatePoint: !updater");
        require(_trader != address(0), "AipxReferralController::updatePoint: invalid address");
    ...
    function claim(uint256 _epoch, address _to) external {
        require(_epoch < currentEpoch, "AipxReferralController::claim: !epoch");
        uint256 reward = claimable(_epoch, msg.sender);
        require(reward != 0, "AipxReferralController::claim: !reward");

        UserInfo storage user = users[_epoch][msg.sender];
        user.claimed += reward;

        AIPX.safeTransfer(_to, reward);

        emit Claimed(_epoch, _to, reward);
    }

    function nextEpoch() external {
        require(enableNextEpoch, "AipxReferralController::nextEpoch: !enableNextEpoch");
        (uint256 nextEpochTimestamp, uint256 _vestingDuration) = getNextEpoch();
        require(block.timestamp >= nextEpochTimestamp, "AipxReferralController::nextEpoch: now < trigger time");
    ...
    function setEnableNextEpoch(bool _enable) external {
        require(msg.sender == distributor, "AipxReferralController::setEnableNextEpoch: !distributor");
    ...
    event AIPXWithdrawn(address indexed to, uint256 amount);
