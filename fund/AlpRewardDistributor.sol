...
contract AlpRewardDistributor is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _pool, address _rewardToken, address _weth, address _ethUnwrapper)
        ...
        initializer
    {
        require(_rewardToken != address(0), "AlpRewardDistributor::initialize: Invalid address");
        require(_pool != address(0), "AlpRewardDistributor::initialize: Invalid address");
        require(_weth != address(0), "AlpRewardDistributor::initialize: Invalid address");
        require(_ethUnwrapper != address(0), "AlpRewardDistributor::initialize: Invalid address");
        __Ownable_init();
        pool = IPool(_pool);
    ...
    function transferRewards(address _to, uint256 _amount) external onlyRequester {
        require(_to != address(0), "AlpRewardDistributor::transferRewards: Invalid address");
    ...
    function transferRewardsToSingleToken(address _to, uint256 _amount, address _tokenOut, uint256 _minAmountOut)
        ...
        onlyRequester
    {
        require(_to != address(0), "AlpRewardDistributor::transferRewardsToSingleToken:invalidAddress");
        require(withdrawableTokens[_tokenOut], "AlpRewardDistributor::transferRewardsToSingleToken:notWithdrawable");
        if (_amount > 0) {
            ...
            require(_amountOut >= _minAmountOut, "AlpRewardDistributor::transferRewardsToSingleToken:slippage");
    ...
    function convertToAlp(address _token, uint256 _amount, uint256 _minAlpAmount) external onlyController {
        require(_token != address(0), "AlpRewardDistributor::convertToAlp: Invalid address");
        if (_amount > 0) {
            uint256 _balanceBefore = rewardToken.balanceOf(address(this));
            IERC20(_token).safeIncreaseAllowance(address(pool), _amount);
            pool.addLiquidity(address(rewardToken), _token, _amount, _minAlpAmount, address(this));
            uint256 _amountOut = rewardToken.balanceOf(address(this)) - _balanceBefore;
            require(_amountOut >= _minAlpAmount, "AlpRewardDistributor::convertToAlp: slippage");
    ...
    function setRequester(address _requester) external onlyOwner {
        require(_requester != address(0), "AlpRewardDistributor::setRequester: Invalid address");
        requester = _requester;
        emit RequesterSet(_requester);
    }

    function recoverFund(address _token, address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "AlpRewardDistributor::recoverFund: Invalid address");
        require(_token != address(0), "AlpRewardDistributor::recoverFund: Invalid address");
    ...
    function recoverETH(address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "AlpRewardDistributor::recoverETH: Invalid address");
        (bool success,) = _to.call{value: _amount}(new bytes(0));
        require(success, "AlpRewardDistributor::recoverETH: ETH transfer failed");
        emit EthRecovered(_to, _amount);
    }

    function setTokenWithdrawable(address _token, bool _allowed) external onlyOwner {
        require(_token != address(0), "AlpRewardDistributor::setTokenWithdrawable: Invalid address");
    ...
    function setController(address _controller) external onlyOwner {
        require(_controller != address(0), "AlpRewardDistributor::setController: Invalid address");
    ...
    event RequesterSet(address indexed _goStaking);
